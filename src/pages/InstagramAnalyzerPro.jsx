...
export default function InstagramAnalyzerPro({ token }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      setError(null);
      const response = await fetchInstagramAnalysis(datasetId, token);
      if (!active) return;
      if (!response?.success) {
        setError(response?.message || response?.error?.message || "Instagram-Analyse konnte nicht geladen werden.");
        setLoading(false);
        return;
      }
